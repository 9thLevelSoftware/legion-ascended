 ...
 *
 * Deliberate, and the one place the mirror departs from the published shape. A
 ...
 * file noisy and teach operators to ignore the check. Values that ARE present
 * are held to the published constraints exactly.
 */
export const settingsSchema = z
  .object({
    ...
    planning: z
      .object({
        max_tasks_per_plan: z.number().int().min(1).max(5).optional(),
    ...
    execution: z
      .object({
        ...
        agent_personality_verbosity: z.enum(["full", "condensed"]).optional(),
    ...
    review: z
      .object({
        default_mode: z.enum(["classic", "panel"]).optional(),
        max_cycles: z.number().int().min(1).max(5).optional(),
        ...
        coverage_thresholds: z.object({}).loose().optional()
      })
      .strict()
      .optional(),
    board: z
      .object({
        default_size: z.number().int().min(2).max(7).optional(),
        min_size: z.number().int().min(2).max(5).optional(),
        discussion_rounds: z.number().int().min(1).max(5).optional(),
        assessment_timeout_ms: z.number().int().min(30_000).optional(),
    ...
    dispatch: z
      .object({
        ...
        fallback_to_internal: z.boolean().optional(),
        timeout_ms: z.number().int().min(10_000).optional(),
    ...
    memory: z
      .object({
        ...
        // does not exist.
        project_scoped_only: z.literal(true).optional(),
    ...
    integrations: z
      .object({
        github: z.enum(["enabled", "disabled", "prompt"]).optional()
      })
      .strict()
      .optional()
  })
