...
- Legacy `.planning` import dry-run/apply/rollback with checksummed backups, explicit review requirement, source preservation, canonical dry-run source roots, lexical and symlink-resolved backup-root safety across repository, `.legion`, planning source, and staging boundaries, plus rollback manifest identity and integrity checks.
...
- `pnpm --filter @legion/legacy-bridge test`: PASS, 32 tests. Evidence: `docs/next/evidence/P02-CLOSEOUT/legacy-bridge-test.log`.
