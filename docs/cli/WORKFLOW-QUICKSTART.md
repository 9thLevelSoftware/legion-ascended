...
Use `legion assess` for a read-only brownfield assessment that prints an evidence-bound report (architecture, findings, assumptions, plan) without modifying source files or runtime configuration. Finding counts are not the audit.
